...
class VerbsAnalyzer:
    ...
    def analyze(self, text: str, processed_text: Dict, text_evaluation: TextEvaluation) -> VerbsEvaluation:
        ...
        for sentence in processed_text['sentences']:
            for token in sentence['tokens']:
                if token['upos'] == "VERB":
                    v = {
                        'token': token,
                        # 'head': if [t for t in sentence['tokens'] if t['id'] == token['head']][0],
                        'children': [t for t in sentence['tokens'] if t['head'] == token['id'] and t['deprel'] in ["aux", "aux:pass", "expl", "expl:impers"]]
                    }
                    verbs.append(v)
